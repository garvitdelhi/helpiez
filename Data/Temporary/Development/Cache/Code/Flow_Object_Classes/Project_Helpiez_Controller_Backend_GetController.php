...
class GetController_Original extends ActionController {
    ...
    public function organisationAction() {
		...
		$this->view->assign("organisation", $inchargeAccount->getOrganisation());
    }
    /**
     * Login Action
     */
...
class GetController extends GetController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('Project\Helpiez\Controller\Backend\GetController' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('Project\Helpiez\Controller\Backend\GetController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('Project\Helpiez\Controller\Backend\GetController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('Project\Helpiez\Controller\Backend\GetController', $propertyName, 'var');
