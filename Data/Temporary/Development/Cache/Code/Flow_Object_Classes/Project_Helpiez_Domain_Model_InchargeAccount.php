 ...
 * @Flow\Entity
 */
class InchargeAccount_Original {
 ...
 */
class InchargeAccount extends InchargeAccount_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface, \TYPO3\Flow\Persistence\Aspect\PersistenceMagicInterface {
		...
		$this->Flow_Aop_Proxy_buildMethodsAndAdvicesArray();
			...
			if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['__construct'])) {
				...
				try {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Project\Helpiez\Domain\Model\InchargeAccount', '__construct', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Project\Helpiez\Domain\Model\InchargeAccount', '__construct', $joinPoint->getMethodArguments());
	 ...
	 public function __clone() {
		...
		$result = NULL;
			...
			try {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Project\Helpiez\Domain\Model\InchargeAccount', '__clone', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Project\Helpiez\Domain\Model\InchargeAccount', '__clone', $joinPoint->getMethodArguments());
					$result = $this->Flow_Aop_Proxy_invokeJoinPoint($joinPoint);

					$advices = $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['__clone']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'];
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Project\Helpiez\Domain\Model\InchargeAccount', '__clone', $joinPoint->getMethodArguments(), NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
	...
	$reflectedClass = new \ReflectionClass('Project\Helpiez\Domain\Model\InchargeAccount');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('Project\Helpiez\Domain\Model\InchargeAccount', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('Project\Helpiez\Domain\Model\InchargeAccount', $propertyName, 'var');
