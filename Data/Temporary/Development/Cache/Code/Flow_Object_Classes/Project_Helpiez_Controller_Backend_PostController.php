...
class PostController_Original extends ActionController {
	...
	/**
	 * @param \Project\Helpiez\Domain\Model\Event $event
	 * @return void
	 */
	public function addEventAction($event){
		if(!$this->authenticationManager->isAuthenticated()) {
			$this->redirect('login', 'Backend\Get');
		}
		...
		$event->setOrganisation($inchargeAccount->getOrganisation());
		$this->eventRepository->add($event);
		$this->redirect('form', 'Backend\Get');
	}

	/**
	 * @param \Project\Helpiez\Domain\Model\Internship $internship
	 * @return void
	 */
	public function addInternshipAction($internship = null){
		if(!$this->authenticationManager->isAuthenticated()) {
			$this->redirect('login', 'Backend\Get');
		}
		$account = $this->securityContext->getAccount();
		...
		$inchargeAccount = $result->getFirst();
		$internship->setOrganisation($inchargeAccount->getOrganisation());
		$this->internshipRepository->add($internship);
		$this->redirect('form', 'Backend\Get');
	}

	/**
	 * @param \Project\Helpiez\Domain\Model\Volunteering $volunteering
	 */
	public function addVolunteeringAction($volunteering){
		if(!$this->authenticationManager->isAuthenticated()) {
			$this->redirect('login', 'Backend\Get');
		}
		$account = $this->securityContext->getAccount();
		...
		$inchargeAccount = $result->getFirst();
		$volunteering->setOrganisation($inchargeAccount->getOrganisation());
		$this->volunteeringRepository->add($volunteering);
		$this->redirect('form', 'Backend\Get');
	}
	 ...
	 */
	public function updateOrganisationAction($organisation){
		$this->organisationRepository->update($organisation);
		$this->redirect('organisation', 'Backend\Get');
	}
...
class PostController extends PostController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('Project\Helpiez\Controller\Backend\PostController' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('Project\Helpiez\Controller\Backend\PostController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('Project\Helpiez\Controller\Backend\PostController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('Project\Helpiez\Controller\Backend\PostController', $propertyName, 'var');
...
$this->Flow_Injected_Properties = array (
  ...
  4 => 'volunteeringRepository',
  5 => 'inchargeAccountRepository',
  6 => 'organisationRepository',
  7 => 'securityContext',
  8 => 'objectManager',
  9 => 'reflectionService',
